document,addEventListener('DOMContentLoaded',() => {
    ...
    (function(index){
    //agrega un "onclick" para cuadro en el tablero
        squares[i].onclick = function(){
        // si cada cuadro debajo del cuadro actual es ocupado vos no lo podes ocupar
            if(squares[index + 7].classList.contains('taken')){
                ...
                //si el cuadro debajo del actual cuadro no esta tomado no podes mover ahi
            } else alert('No podes mover ahi')
        }
    })(i)

    //mostrar tablero para ganador o perdedor
    //check the board for a win or lose
    function checkBoard() {
        //hacer la constate del la array ganadora
        const winningArrays = [
        ...
        //ahora tomamos los 4 valores en cada "winningArray" y los introdicimos en los valores del los cuadros 
        for(let y = 0; y < winningArrays.length; y++) {
            ...
            //ahora mostramos esos arrays para ver si todos ellos tienen la clase de player-one
            if(square1.classList.contains('player-one') &&
                square2.classList.contains('player-one') &&
                square3.classList.contains('player-one') &&
                square4.classList.contains('player-one')) {
                    //si lo tienen el player-one pasa a ganar el juego  
                    result.innerHTML = 'Jugador uno GANA!'

            } 
            //ahora mostramos si tienen el classname player-two
            else if (square1.classList.contains('player-two') &&
                square2.classList.contains('player-two') &&
                square3.classList.contains('player-two') &&
                square4.classList.contains('player-two')) {
                    //si lo tiene, el player-two pasa a ganar el juego 
                    result.innerHTML = 'Jugador dos GANA!!'
            }
        }
    }

    //agregamos event listener a cuada cuadro que activa el checkBoard en la function on click
